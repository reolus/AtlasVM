...
def vm_attach_iso(
    ...
    try:
        lv.attach_iso(name, iso_path, live=live)
        log_event(db, user, 'attach_iso', name, iso_path)
        finish_task(db, task, 'success', 'ISO attached')
        return _redirect(f'/vms/{name}', message='ISO attached')
    except Exception as exc:
        finish_task(db, task, 'failed', str(exc))
        log_event(db, user, 'attach_iso_failed', name, str(exc))
...
def vm_eject_iso(
    ...
    try:
        lv.eject_iso(name, live=live)
        log_event(db, user, 'eject_iso', name, 'Ejected CD-ROM media')
        finish_task(db, task, 'success', 'ISO ejected')
        return _redirect(f'/vms/{name}', message='ISO ejected')
    except Exception as exc:
        finish_task(db, task, 'failed', str(exc))
        log_event(db, user, 'eject_iso_failed', name, str(exc))
        return _redirect(f'/vms/{name}', error=str(exc))
    finally:
        lv.close()
...
def vm_action(request: Request, name: str, action: str, db: Session = Depends(get_db), user: str = Depends(require_operator)):
    ...
    try:
        ...
        if action == 'start':
            ...
            if action == 'template':
                LibvirtService().mark_vm_as_template(name)
                return RedirectResponse(url=f'/vms/{name}', status_code=303)
...
def task_kill(task_id: int, db: Session = Depends(get_db), user: str = Depends(require_operator)):
    """Mark a hung AtlasVM task as failed.

    This clears stale task_log rows. It does not kill an OS process because
    task_log does not store a process ID.
    """
    import sqlite3
    from datetime import datetime

    conn = sqlite3.connect('/opt/atlasvm/atlasvm.db')
    conn.row_factory = sqlite3.Row

    try:
        row = conn.execute(
            "select id, action, target, status, message from task_log where id = ?",
            (task_id,),
        ).fetchone()

        if row is None:
            return _redirect('/tasks', error=f'Task {task_id} was not found')

        if row['status'] != 'running':
            return _redirect('/tasks', error=f'Task {task_id} is not running')

        finished_at = datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')
        old_message = row['message'] or ''
        message = f'Marked failed by {user}. Previous message: {old_message}'.strip()

        conn.execute(
            "update task_log set status = ?, finished_at = ?, message = ? where id = ? and status = ?",
            ('failed', finished_at, message, task_id, 'running'),
        )
        conn.commit()

        try:
            log_event(db, user, 'kill_task', str(task_id), f"{row['action']} on {row['target']} marked failed")
        except Exception:
            pass

        return _redirect('/tasks', message=f'Task {task_id} marked failed')
    finally:
        conn.close()
