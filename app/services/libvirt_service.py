...
class LibvirtService:
    ...
    def update_vm_basic(self, name: str, memory_mb: int, vcpus: int, description: str | None = None) -> dict[str, Any]:
        """Update basic VM settings without undefining/redefining the domain.

        Snapshot-safe behavior:
        - Description is updated with libvirt metadata.
        - Memory and vCPU config are updated through libvirt setters.
        - Memory/vCPU changes still require the VM to be shut down.
        """
        domain = self.conn.lookupByName(name)
        ...
        if memory_changed:
            ...
            try:
                domain.setMemoryFlags(memory_kib, flags)
            except Exception:
                # Some drivers need max memory updated separately while inactive.
                domain.setMaxMemory(memory_kib)
                domain.setMemoryFlags(memory_kib, flags)

        if vcpu_changed:
            domain.setVcpusFlags(vcpus, flags)

        marker = '[ATLASVM_TEMPLATE]'

        try:
            current_desc = domain.metadata(
            ...
            current_desc = root.findtext('./description') or ''

        was_template = marker in current_desc
        new_desc = (description or '').strip()

        if was_template and marker not in new_desc:
            if new_desc:
                new_desc = new_desc + '\n\n' + marker
            else:
                new_desc = marker
    ...
    def attach_iso(self, name: str, iso_path: str, live: bool = False) -> None:
        """Attach ISO media without undefining/redefining the VM.

        Snapshot-safe behavior:
        - stopped VM: updates persistent config
        - running VM with live=True: updates live device and persistent config
        - running VM with live=False: refuses and tells the caller what to do
        """
        ...
        if domain.isActive() and not live:
            raise RuntimeError('VM is running. Enable live ISO attach/eject or shut the VM down first.')
        ...
        if cdrom is not None:
            ...
            if target is not None:
                target_dev = target.attrib.get('dev') or target_dev
                target_bus = target.attrib.get('bus') or target_bus

        cdrom_xml = f"""
        <disk type='file' device='cdrom'>
          <driver name='qemu' type='raw'/>
          <source file='{html.escape(iso_path)}'/>
          <target dev='{html.escape(target_dev)}' bus='{html.escape(target_bus)}'/>
          <readonly/>
        </disk>
        """
        ...
        if cdrom is None:
            ...
            domain.updateDeviceFlags(cdrom_xml, flags)


    def eject_iso(self, name: str, live: bool = False) -> None:
        """Eject ISO media without undefining/redefining the VM."""
        domain = self.conn.lookupByName(name)
        ...
        target = cdrom.find('./target')
        target_dev = target.attrib.get('dev') if target is not None else 'sda'
        target_bus = target.attrib.get('bus') if target is not None else 'sata'
    ...
    def add_disk(self, name: str, size_gb: int, storage_pool: str | None = None) -> dict[str, Any]:
        """Add a disk without undefining/redefining the VM.

        Snapshot-safe behavior:
        - Creates the backing volume.
        - Attaches the disk XML using attachDeviceFlags.
        - Rolls back the newly-created volume if attach fails.
        - Does not call _redefine_domain() or defineXML() on the VM.
        """
        domain = self.conn.lookupByName(name)
        ...
        for candidate_index in range(disk_index, disk_index + 50):
            candidate_name = f'{name}-disk{candidate_index}'
            try:
                volume = create_volume_for_pool(storage_pool, candidate_name, size_gb, prefer_format='qcow2')
                disk_index = candidate_index
                break
            except Exception as exc:
                last_error = exc
                if 'exists already' in str(exc) or 'already exists' in str(exc):
                    continue
                raise

        if volume is None:
            raise RuntimeError(f'Could not create a unique disk volume for {name}: {last_error}')

        used_targets = set()
        try:
            xml_text_for_targets = domain.XMLDesc(libvirt.VIR_DOMAIN_XML_INACTIVE)
        except Exception:
            xml_text_for_targets = domain.XMLDesc()

        target_root = ET.fromstring(xml_text_for_targets)
        for disk_node in target_root.findall('./devices/disk'):
            target_node = disk_node.find('./target')
            if target_node is not None and target_node.attrib.get('dev'):
                used_targets.add(target_node.attrib.get('dev'))

        target_dev = None
        for letter in 'bcdefghijklmnopqrstuvwxyz':
            candidate_dev = f'vd{letter}'
            if candidate_dev not in used_targets:
                target_dev = candidate_dev
                break

        if target_dev is None:
            try:
                delete_disk_source(volume.get('path') or volume.get('source') or '', pool_name=storage_pool)
            except Exception:
                pass
        ...
        return {
            'target': target_dev,
            'volume': volume.get('name') or f'{name}-disk{disk_index}',
            'path': volume.get('path') or volume.get('source') or '',
            'pool': storage_pool,
            'format': volume.get('format') or 'qcow2',
            'mode': 'config',
        }
    ...
    def set_template(self, name: str, enabled: bool = True) -> dict[str, Any]:
        """Mark or unmark a VM as an AtlasVM template.

        Snapshot-safe implementation:
        - Does not undefine/redefine the domain.
        - Updates only the libvirt description metadata.
        - Requires the VM to be shut down.
        """
        domain = self.conn.lookupByName(name)
    ...
    def _redefine_domain(self, domain: libvirt.virDomain, root: ET.Element) -> None:
        ...
        Undefine/redefine breaks VMs with snapshots and must not be used for
        routine VM edits. Keep this guard so accidental legacy calls fail loudly
        instead of corrupting a perfectly innocent Tuesday.
        """
        name = domain.name() if domain is not None else 'unknown'
        raise RuntimeError(f'_redefine_domain is disabled for VM {name}; use snapshot-safe libvirt update APIs instead')
    ...
    def mark_vm_as_template(self, vm_name: str) -> dict:
        """Backward-compatible wrapper for older callers.

        Kept so old routes/scripts do not break, but it now delegates to the
        snapshot-safe metadata implementation instead of redefining the domain.
        """
        result = self.set_template(vm_name, True)
        return {
            "ok": True,
            "vm": vm_name,
            "template": True,
            "message": f"{vm_name} was marked as a template.",
            **result,
        }
